...
df = df %>% filter(State == "California") %>% group_by(FIPS, County) %>%
    summarise(
      ...
      wtd_prop_hosp_in_icu = weighted.mean(critical_case_rate/(critical_case_rate+severe_cases_rate), population_in_age_group))
...
# use on our data
long_dt[,icu_census := get_hospitalizations(est_icu_hosp, ICU_LOS, doubling_time), by = c('County', 'doubling_time')]
long_dt[,acute_census := get_hospitalizations(est_acute_hosp, ACUTE_LOS, doubling_time), by = c('County', 'doubling_time')]
```
